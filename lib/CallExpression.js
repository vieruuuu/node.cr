...
function CallExpression(_expression, EOL = eol, prev = null, indent) {
  ...
  if (isConsoleLog(expression.callee)) {
    ...
    result += `${require("./parseLiterals")(expression.arguments[0])}.to_s()`;
    ...
    result += `${require("./parseLiterals")(expression.callee.object)}.gsub${args}`;
  } else if (isParseInt(expression.callee)) {
    let radix;
    try {
      radix = require("./parseLiterals")(expression.arguments[1]);
    } catch (err) {
      radix = 10;
    }
    ...
    result += `${require("./parseLiterals")(expression.arguments[0])}.to_f()`;
